...
// <!-- FINAL-FIX: Re-added signInAnonymously -->
import { 
  getAuth, 
  onAuthStateChanged, 
...
// 2. Add a user (hattokyherbalcare@gmail.com)
// 3. Copy the "User UID" from that new account and paste it here.
const ADMIN_USER_ID = "KD63qdJ0MkT4G3VSigQ2yUJBkjH2";

// --- Global Setup (MANDATORY FIREBASE VARIABLES) ---
const appId = typeof __app_id !== 'undefined' ? __app_id : 'default-app-id';
const firebaseConfig = typeof __firebase_config !== 'undefined' ? JSON.parse(__firebase_config) : {};
// This is the auth token for the *chat environment*
...
const App = () => {
  ...
  // <!-- FINAL-FIX: IS_ADMIN check is now based on the user's UID -->
  // An anonymous user's UID will NOT match the ADMIN_USER_ID.
      ...
      if (Object.keys(firebaseConfig).length > 0) {
        ...
        const unsubscribe = onAuthStateChanged(authInstance, (currentUser) => {
          if (currentUser) {
            ...
            // This should not happen now, as we re-login anonymously
            setUser(null);
          }
          setLoading(false); // Auth check is complete
        });

        // <!-- FINAL-FIX: Automatically sign in the user (as guest or with token) -->
        const authenticate = async () => {
          if (initialAuthToken) {
            // We are in the chat environment with a token
            await signInWithCustomToken(authInstance, initialAuthToken);
          } else {
            // We are on a deployed site OR in a preview without a token.
            // Sign in as a guest so we can read products.
  ...
  useEffect(() => {
    // <!-- FINAL-FIX: Wait for 'user' state to be set before fetching -->
    // This ensures we are authenticated (even as a guest) before trying to read
    if (!db || !user) return; 
  ...
  }, [db, user]); // <!-- FINAL-FIX: Re-run this when 'user' is set
...
I will proceed with payment using the Order ID as reference. Please confirm availability!
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // Success! onAuthStateChanged will trigger,
      // which will set the user and close the modal.
      setShowLogin(false);
      setLoginEmail('');
      setLoginPassword('');
    } catch (err) {
  ...
  const handleLogout = async () => {
    ...
    await signOut(auth);
    // <!-- FINAL-FIX: After signing out, sign back in as a guest -->
    await signInAnonymously(auth);
    // onAuthStateChanged will trigger and set the new anonymous user.
  };
        ...
        <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 mb-10">
            ...
            <form onSubmit={handleAddProduct} className="space-y-4">
                ...
                <input
                    type="number"
                    name="price"
                    placeholder={`Price (e.g., 85.50) in ${CURRENCY_SYMBOL}`}
                ...
                <input
                    type="number"
                    name="initialStock"
                    placeholder="Initial Stock Quantity (e.g., 50)"
                ...
                <input
                    type="url"
                    name="imageUrl"
                    placeholder="Image URL (from Cloudinary/Storage)"
                ...
                <button
                    ...
                    className={`w-full py-3 bg-green-500 hover:bg-green-600 text-white font-bold rounded-xl flex items-center justify-center space-x-2 disabled:opacity-50`}
                >
                            ...
                            <div className="flex justify-between items-start mb-2">
                                ...
                                <div className="flex items-center space-x-2">
                                    ...
                                    <select
                                        ...
                                        className={`px-3 py-1 rounded-full text-sm font-semibold border ${order.status === 'PENDING' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' : order.status === 'PAID' ? 'bg-green-100 text-green-800 border-green-300' : 'bg-gray-100 text-gray-800 border-gray-300'}`}
                                    >
  ...
  const renderProductCatalog = (products, handleAddToCart) => (
    <>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 pb-20"> 
                ...
                return (
                    <div key={product.id} className="bg-white p-4 rounded-2xl shadow-xl transition transform hover:scale-[1.02] duration-300 flex flex-col justify-between border border-gray-100">
                        <img
                            ...
                            alt={product.name}
                            className="w-full h-40 object-cover rounded-lg mb-3 shadow-inner"
                            onError={(e) => {
                                e.target.onerror = null;
                                e.target.src = `https://placehold.co/400x300/60a5fa/ffffff?text=Product+Image`;
                            }}
                        />
                        ...
                        <button
                            ...
                            className={`flex items-center justify-center space-x-2 text-white font-semibold py-2.5 px-4 rounded-xl transition duration-200 ${
                                isAvailable 
            ...
            // <!-- FINAL-FIX: Updated the "No products" message -->
            }) : (
            ...
            {cartItems.length === 0 ? (
                ...
                <>
                            ...
                            <div key={item.id} className="flex items-center justify-between p-4 border-b border-gray-100 bg-white hover:bg-gray-50 transition duration-150">
                                ...
                                <div className="flex items-center space-x-2 border border-gray-200 rounded-full p-0.5">
                                    <button
                                        ...
                                        className="bg-gray-100 hover:bg-gray-300 w-7 h-7 rounded-full text-base transition duration-150 text-gray-700"
                                    >
                                        −
                                    </button>
                                    ...
                                    <button
                                        onClick={() => handleUpdateQuantity(item.id, 1)}
                                        className={`bg-indigo-500 hover:bg-indigo-600 w-7 h-7 rounded-full text-white text-base transition duration-150`}
                                    >
                                        +
                                    </button>
                                </div>
                    ...
                    <button
                        ...
                        className={`w-full mt-6 flex items-center justify-center space-x-2 bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-xl text-lg transition duration-200 shadow-lg shadow-indigo-200`}
                    >
                        <CheckCircle size={20} />
        ...
        <form onSubmit={handlePlaceOrder} className="space-y-5">
            ...
            <div className="space-y-4">
                {[
                    ...
                    { name: 'phone', type: 'tel', placeholder: 'Phone Number (e.g., 080...)', icon: Phone },
                    { name: 'location', type: 'text', placeholder: 'Delivery Location/Address', icon: MapPin },
                ...
                <textarea
                    placeholder="Specific Order Notes (Optional)"
            ...
            <div className="p-4 bg-gray-100 rounded-xl border border-gray-200">
                ...
                <div className="text-sm space-y-1">
                    ...
                    <p><strong>Account Name:</strong> {Hattoky Herbal Care}</p>
            ...
            <button
                ...
                className="w-full flex items-center justify-center space-x-3 bg-green-500 hover:bg-green-600 text-white font-extrabold py-3 rounded-xl text-xl transition duration-200 shadow-xl shadow-green-300 disabled:opacity-50"
                onClick={() => setOrderId(orderId || generateOrderId())}
            >
                {loading ? (
                    <>
                    ...
                    <>
                    <Send size={24} />
                    <span>Place Order & Send on WhatsApp ({CURRENCY_SYMBOL}{cartTotal.toFixed(2)})</span>
                    </>
                )}
            </button>
        </form>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans p-4 sm:p-8">
      ...
      <Modal isOpen={!!error} title="System Error" onClose={() => setError(null)}>
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          ...
          {IS_ADMIN && (
              <button
                ...
                className={`px-3 py-2 rounded-xl font-semibold transition duration-150 text-sm sm:text-base ${view === 'ADMIN' ? 'bg-red-600 text-white shadow-lg' : 'bg-red-100 text-red-600 hover:bg-red-200'}`}
              >
          ...
          <button
            ...
            className={`px-3 py-2 rounded-xl font-semibold transition duration-150 text-sm sm:text-base ${view === 'PRODUCTS' ? `bg-indigo-600 text-white shadow-lg` : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
          >
            <List size={20} className="inline sm:mr-1" /> <span className="hidden sm:inline">Goods</span>
          </button>
          <button
            onClick={() => setView('CART')}
            className={`relative px-3 py-2 rounded-xl font-semibold transition duration-150 text-sm sm:text-base ${view === 'CART' || view === 'FORM' ? `bg-indigo-600 text-white shadow-lg` : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
          >
          ...
          {/* <!-- FINAL-FIX: This logic is now correct --> */}
          {/* If user is admin, show Logout. If not, show Login. */}
          {IS_ADMIN ? (
             <button
                onClick={handleLogout}
                className={`px-3 py-2 rounded-xl font-semibold transition duration-150 text-sm sm:text-base bg-gray-100 text-gray-700 hover:bg-gray-200`}
                title="Logout"
              >
              ...
              <button
                ...
                className={`px-3 py-2 rounded-xl font-semibold transition duration-150 text-sm sm:text-base bg-green-100 text-green-700 hover:bg-green-200`}
                title="Admin Login"
              >
...
const MOCK_PRODUCTS = [
  { id: 'p1', name: 'Artisan Crafted Mug', price: 25.00, imageUrl: 'https://placehold.co/400x300/60a5fa/ffffff?text=Mug', description: 'Hand-painted ceramic, limited edition.', stock: 10, createdAt: { seconds: 1678886400 } },
  { id: 'p2', name: 'Luxury Scented Candle', price: 150.50, imageUrl: 'https://placehold.co/400x300/a78bfa/ffffff?text=Candle', description: 'Sandalwood and patchouli fragrance.', stock: 5, createdAt: { seconds: 1678886300 } },
  { id: 'p3', name: 'Minimalist Wall Clock', price: 45.99, imageUrl: 'https://placehold.co/400x300/c7d2fe/ffffff?text=Clock', description: 'Silent sweep mechanism, matte black finish.', stock: 0, createdAt: { seconds: 1678886200 } },
];
